...
### Ваша находка №6
- ruby-prof в режиме call stack на файле data3 c 1_000_000 строк показал наибольшую точку роста в методе collect_stats_from_users который вызывает очень много методов map(460_000). Попытаемся сократить их.
  Для начала избавимся от
  ```
  times = user.sessions.map{|s| s['time'].to_i}
  browsers = user.sessions.map {|s| s['browser'].upcase}.sort
  ```
  Вынесем их преобразования в `parse_session` и сделаем один обход вместо двух
  После всех преобразований количество вызовов map уменьшилось до 153_000, а процент уменьшился до 30 против 40 изначальных
  Время обработки файла data_large.txt упала незначительно и стала 87 sec
